...
每个 skill 安装后，其 `name`、`description` 和 `location` 会被注入 LLM 上下文，作为 agent 框架判断是否加载 skill body 的依据。这部分元数据虽然轻量，但积少成多：

| 已启用 skill 数量 | 约消耗上下文 | 对 200K 窗口的影响 | 对 1M 窗口的影响 |
|---|---|---|---|
| ~10 个 | ~1.5-2% | 几乎无感 | 无感 |
| ~50 个 | ~7.5-10% | 有感知但不影响使用 | 几乎无感 |
| ~100 个 | ~15-20% | 需要取舍 | 轻微 |
